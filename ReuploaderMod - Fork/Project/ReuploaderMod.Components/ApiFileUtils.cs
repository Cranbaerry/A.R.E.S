...
namespace ReuploaderMod.Components
{
    ...
    public class ApiFileUtils : MonoBehaviour
    {
        ...
        public ApiFileUtils(System.IntPtr intptr_1)
    : base(intptr_1)
        {
        ...
        public ApiFileUtils(System.Delegate refdelegate, System.IntPtr methodinfo)
            : base(ClassInjector.DerivedConstructorPointer<ApiFileUtils>())
        {
        ...
        public static bool DebugDetails = true;

        public static void DebugLogWarning(string text)
        {
            if (DebugDetails)
            {
                MelonLogger.LogWarning("[APIFile Debug] : "+ text);
            }
        }
        ...
        public static void DebugLog(string text)
        {
            if (DebugDetails)
            {
                MelonLogger.Log("[APIFile Debug] : "+ text);
            }
        }
        public static void DebugLogError(string text)
        {
            if (DebugDetails)
            {
                MelonLogger.LogError("[APIFile Debug] : "+ text);
            }
        }
        ...
        public IEnumerator UploadFile(string filename, string existingFileId, string friendlyName,
            OnFileOpSuccess onSuccess, OnFileOpError onError, OnFileOpProgress onProgress, FileOpCancelQuery cancelQuery)
        {
            DebugLog("UploadFile: filename: " + filename + ", file id: " +
            ...
            while (true)
            {
                ...
                if (!string.IsNullOrEmpty(errorStr))
                {
                    ...
                    {
                        DebugLogError("Couldn't find file record: " + existingFileId + ", creating new file record");
            ...
            if (apiFile.HasExistingOrPendingVersion())
            {
                ...
                if (string.Compare(fileMD5Base64, apiFile.GetFileMD5(apiFile.GetLatestVersionNumber())) == 0)
                {
                    ...
                    else
                    {
                        ...
                        DebugLog("Retrying previous upload");
                    }
                }
                else
                {
            ...
            if (EnableDeltaCompression)
                DebugLog("Delta size " + deltaFileSize + " (" + ((float)deltaFileSize / (float)fullFizeSize) + " %), full file size " + fullFizeSize + ", uploading " + (uploadDeltaFile ? " DELTA" : " FULL FILE"));
            else
                DebugLog("Delta compression disabled, uploading FULL FILE, size " + fullFizeSize);
            ...
            if (isPreviousUploadRetry)
            {
                ...
                if (isValid)
                {
                    ...
                    DebugLog("Using existing version record");
                }
                else
                {
            ...
            if (uploadDeltaFile)
            {
                if (apiFile.GetLatestVersion().delta.status == VRC.Core.ApiFile.Status.Waiting)
                {
                    ...
                    yield return MelonCoroutines.Start(UploadFileComponentInternal(apiFile,
                        ...
                        {
                            DebugLog("Successfully uploaded file delta.");
                            apiFile = file;
                        },
                        delegate (string error)
                        {
            ...
            else
            {
                if (apiFile.GetLatestVersion().file.status == VRC.Core.ApiFile.Status.Waiting)
                {
                    ...
                    yield return MelonCoroutines.Start(UploadFileComponentInternal(apiFile,
                        ...
                        delegate (VRC.Core.ApiFile file)
                        {
                            DebugLog("Successfully uploaded file.");
                            apiFile = file;
                        },
                        delegate (string error)
                        {
            ...
            if (apiFile.GetLatestVersion().signature.status == VRC.Core.ApiFile.Status.Waiting)
            {
                ...
                yield return MelonCoroutines.Start(UploadFileComponentInternal(apiFile,
                    ...
                    delegate (VRC.Core.ApiFile file)
                    {
                        DebugLog("Successfully uploaded file signature.");
                        apiFile = file;
                    },
                    delegate (string error)
                    {
        ...
        private static void LogApiFileStatus(VRC.Core.ApiFile apiFile, bool checkDelta, bool logSuccess = false)
        {
            if (apiFile == null || !apiFile.IsInitialized)
            {
                ...
                DebugLog("< color = yellow > Processing { 3}: { 0}, { 1}, { 2}</ color > " +
            ...
            if (apiFile != null && apiFile.IsInitialized && logSuccess)
            {
                ...
                if (apiFields != null)
                    DebugLog("<color=yellow>{0}</color>" + VRC.Tools.JsonEncode(apiFields));
            }
        }
        ...
        {
            DebugLog("CreateOptimizedFile: " + filename + " => " + outputFilename);
            ...
            {
                DebugLog("CreateOptimizedFile: (not gzip compressed, done)");
                // nothing to do
                if (onSuccess != null)
        ...
        {
            DebugLog("CreateFileSignature: " + filename + " => " + outputSignatureFilename);
        ...
        {
            DebugLog("CreateFileDelta: " + newFilename + " (delta) " + existingFileSignaturePath + " => " + outputDeltaFilename);
        ...
        protected static void Success(OnFileOpSuccess onSuccess, VRC.Core.ApiFile apiFile, string message)
        {
            ...
            DebugLog("ApiFile " + apiFile.ToStringBrief() + ": Operation Succeeded!");
            if (onSuccess != null)
        ...
        protected static void Error(OnFileOpError onError, VRC.Core.ApiFile apiFile, string error, string moreInfo = "")
        {
            ...
            DebugLogError("ApiFile " + apiFile.ToStringBrief() + ": Error: " + error + "\n" + moreInfo);
            if (onError != null)
        ...
        protected static bool CheckCancelled(FileOpCancelQuery cancelQuery, OnFileOpError onError, VRC.Core.ApiFile apiFile)
        {
            if (apiFile == null)
            {
                DebugLogError("apiFile was null");
                return true;
            }

            if (cancelQuery != null && cancelQuery(apiFile))
            {
                DebugLog("ApiFile " + apiFile.ToStringBrief() + ": Operation cancelled");
                if (onError != null)
        ...
        private bool uploadFileComponentValidateFileDesc(VRC.Core.ApiFile apiFile, string filename, string md5Base64, long fileSize, VRC.Core.ApiFile.Version.FileDescriptor fileDesc, Action<VRC.Core.ApiFile> onSuccess, Action<string> onError)
        {
            if (fileDesc.status != VRC.Core.ApiFile.Status.Waiting)
            {
                ...
                DebugLog("UploadFileComponent: (file record not in waiting status, done)");
                if (onSuccess != null)
                    onSuccess(apiFile);
                return false;
            }
        ...
        private IEnumerator uploadFileComponentDoMultipartUpload(VRC.Core.ApiFile apiFile, VRC.Core.ApiFile.Version.FileDescriptor.Type fileDescriptorType, string filename, string md5Base64, long fileSize, Action<VRC.Core.ApiFile> onSuccess, Action<string> onError, Action<long, long> onProgess, FileOpCancelQuery cancelQuery)
        {
            ...
            VRC.Core.ApiFile.UploadStatus uploadStatus = null;
            {
                while (true)
                {
                    ...
                    apiFile.GetUploadStatus(apiFile.GetLatestVersionNumber(), fileDescriptorType, (System.Action<ApiContainer>)delegate (ApiContainer c)
                    {
                        ...
                        wait = false;
                        DebugLog("Found existing multipart upload status (next part = " + uploadStatus.nextPartNumber + ")");
                    }, (System.Action<ApiContainer>)delegate (ApiContainer c)
                    {
            ...
            while (true)
            {
                ...
                string errorStr = "";
                bool worthRetry = false;
                apiFile.FinishUpload(fileDescriptorType, etags.ConvertToIl2cpp(), (System.Action<ApiContainer>)delegate (ApiContainer c)
                {
                    MelonLogger.LogWarning("YOU CAN IGNORE THE FOLLOWING CASTING ERROR!");
                    apiFile = c.Model.Cast<VRC.Core.ApiFile>();
                    wait = false;
        ...
        {
            DebugLog("UploadFileComponent: " + fileDescriptorType + " (" + apiFile.id + "): " + filename);
