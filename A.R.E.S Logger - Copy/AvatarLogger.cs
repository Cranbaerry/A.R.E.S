...
namespace AvatarLogger
{
    ...
    public class AvatarLogger : MelonMod
    {
        ...
        public override void OnSceneWasLoaded(int buildIndex, string sceneName)
        {
            if (sceneName == "ui")
            {
                mainPage = new MenuPage("Apollo_MainMenu" /* GameObject Name */, "Apollo" /* Page Title */, true /* isRootPage (only set to true when it the menu is opened from the main quickmenu page */, false /* useBackButton */, true /* useTopRightButton */, delegate { } /* topRightButtonAction */, "Click to join the official Apollo Discord Server!" /* topRightButtonToolTip */, ButtonImage /* topRightButtonSprite*/);
                new Tab(APIStuff.GetMenuTabTemplate().transform.parent /* transformOfParent */, "Apollo_MainMenu" /* nameOfMenuToOpen (gameobject name) */, "Apollo Utility Mod" /* tabButtonToolTip */, ButtonImage /* tabButtonIcon */);
                var tweaksGroup = new ButtonGroup(mainPage.menuContents, "Adjustments");
                new SingleButton(tweaksGroup, "Modules", Modules.OpenMenu, "Menus of Apollo's Modules", ButtonImage);
            }
        }
