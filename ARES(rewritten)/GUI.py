...
class Ui(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        self.Outdated = self.findChild(QtWidgets.QPushButton, 'OutdatedButton')
        self.Outdated.clicked.connect(UpdatePushed)
        self.Outdated.hide()
        ...
        #Update checking(disabled if in debug mode!)
        if DebugMode == False:
            if VersionCheck(version) == False:
                self.Outdated.show()
                self.Outdated.setStyleSheet("background-color: red; border: 3px solid black;")
    ...
    def PatchEMMWrapper(self):
        try:
            ...
            threading.Thread(target=PatchEMM, args={self.ProgBar}).start()
            self.LogWrapper("Thread closed and EMM patched!")
    ...
    def OpenUnityWrapper(self):
        try:
            ...
            threading.Thread(target=OpenUnity, args={self.Settings["Unity_Exe"]}).start()
            self.LogWrapper("Thread closed and unity opened!")
            #Enables button again
    ...
    def HotswapWrapper(self,vrca):
        try:
            ...
            threading.Thread(target=Hotswap, args={self.ProgBar}).start()
            self.LogWrapper("Thread closed and hotswap complete!")
            #Enables button again
            self.Hotswap.setEnabled(True)
        except:
    ...
    def RepairVRCAWrapper(self):
        try:
            ...
            threading.Thread(target=RepairVRCA, args={self.ProgBar}).start()
            self.LogWrapper("Thread closed and repair complete!")
            # Enables button again
            self.RepairVRCA.setEnabled(True)
        except:
    ...
    def ExtractVRCAWrapper(self):
        try:
            ...
            threading.Thread(target=ExtractVRCA, args={ExtValue}).start()
            self.LogWrapper("VRCA extracted, thread closed!")
